  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!otpSent) {
        ...
        const res = await api.post('/auth/register-with-otp', { name, email, password, otp });
        if (isMounted.current) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {info && (
          <div style={{ padding: '12px 16px', background: '#ecfeff', color: '#164e63', borderRadius: 8, marginBottom: 16, fontSize: 14 }}>
            {info}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            ...
            <div style={{ position: 'relative' }}>
              ...
              />
              <button type="button" onClick={() => setShowPwd(v => !v)} style={{ position: 'absolute', right: 8, top: 8, padding: '6px 8px', background: '#111', color: '#fff', borderRadius: 6 }}>
                {showPwd ? 'Hide' : 'Show'}
              </button>
            </div>
          ...
          {otpSent && (
            <div className="form-group">
              ...
              <input
                ...
                className="form-input"
                required
                style={{ fontSize: '16px', padding: '14px 16px', letterSpacing: '4px' }}
              />
              <div style={{ fontSize: '12px', color: '#64748b', marginTop: 4 }}>We emailed a 4-digit OTP. It is valid for 5 minutes.</div>
            </div>
          )}
